"""
此程序尝试增加一个新的结构，使用output，增加全连接层，从而对比与原始数据的误差，正在更改
2022/1/3
...
class MultiHeadAttention(nn.Module):
    def __init__(self):
        ...
        self.head_fc = nn.Linear(n_heads, 1).to(device)
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        #增加一个全连接层，获取多头的权重
        context_head6 = context.transpose(1, 3)
        context_head1 = self.head_fc(context_head6)
        context_head1 = context_head1.transpose(1, 3)
        headV_weight = self.head_fc.weight


        context = context_head1.transpose(1, 2).contiguous().view(batch_size, -1,
                                                            1 * d_v)  # context: [batch_size, seq_len, n_heads, d_v]
        output = self.linear(context)
        attliner = output
        out = self.LayerNorm(output + residual)

        # return nn.LayerNorm(d_model)(output + residual)  # output: [batch_size, seq_len, d_model]
        return out, attn,attv,attliner,headV_weight
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        enc_outputs, attn,attv,attliner ,headV_weight= self.enc_self_attn(enc_inputs, enc_inputs, enc_inputs,
                                               enc_self_attn_mask)  # enc_inputs to same Q,K,V
        attention_outputs = enc_outputs
        enc_outputs = self.pos_ffn(enc_outputs)  # enc_outputs: [batch_size, seq_len, d_model]

        return enc_outputs, attention_outputs, attn,attv,attliner,headV_weight
...
class BERT(nn.Module):
    ...
    def forward(self, input_ids, segment_ids, masked_pos):
        ...
        for layer in self.layers:
            output, attention_output, enc_self_attn,attv,attliner ,headV_weight= layer(output, enc_self_attn_mask)
        ...
        return logits_lm, logits_clsf, h_pooled, output, attention_output, enc_self_attn,attv,attliner,headV_weight
...
if __name__ == '__main__':
    ...
    # 这是把四维nii数据转成二维数据的代码
    #masker = NiftiMasker(mask_img='data/ADHD200_mask_152_4mm.nii.gz')
    masker = NiftiMasker()
    ...
    print(pre_list)
    # for j in range(3000):
    #     newlist = train_list.remove(pre_list[j])
    # print(train_list)
    ...
    for epoch in range(epochsize):
        ...
        for input_ids, segment_ids, masked_tokens, masked_pos in loader:
            ...
            logits_lm, logits_clsf, h_pooled, output, attention_output,enc_self_attn,attv,attliner,headV_weight = model(input_ids, segment_ids, masked_pos)
        ...
        #dev_loss = evaluate(model, loader_pre)
        #loss_val_all.append(dev_loss)
        print('Epoch:', '%04d' % (epoch + 1), 'loss =', '{:.6f}'.format(train_loss))
    end = datetime.datetime.now()
    headV_weight = headV_weight.cpu().data.numpy()
    np.save('headV_weight.npy', headV_weight)
    print("训练时间为")
    print(end-start)
    epochnum = np.arange(0, epochsize, 1)
    plt.plot(epochnum, loss_train_all, 'b')
    #plt.plot(epochnum, loss_val_all, 'r')
